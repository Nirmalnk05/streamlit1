...
def main():
    ...
    if choice == "Model Training":
        if st.button("Start Train") :
            allResults = glob.glob('/Users/nirmalnk/Downloads/batch31-38_with_target_diff_prx_2000tampered/*.csv',
                                   recursive=True)
            allResults = sorted(allResults, key=lambda x: (x.split("/")[-1]))
            #st.write(allResults)

            newpath1 = '/Users/nirmalnk/Downloads/batch31-38_with_target_diff_prx_2000tampered/'
            ...
            for i, j in enumerate(dff['file']):
                # print(i,j)
                df = pd.read_csv('/Users/nirmalnk/Downloads/batch31-38_with_target_diff_prx_2000tampered/' + j)
                head, tail = os.path.split(j)
                # print(i,df.shape[1])
